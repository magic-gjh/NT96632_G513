...
static void LIS331DL_Write2Byte(UINT32 *buf)
{
    ...
    debug_msg("LIS331DL_Write2Byte[%x] = %x\r\n",buf[0],buf[1]);
...
static void LIS331DL_Write1BRead1B(UINT32 *buf)
{
    ...
    spi_setCSActive(FALSE);
    debug_msg("LIS331DL_Write1BRead1B[%x] = %x\r\n",buf[0],buf[1]);	
}
...
{
 UINT32 buf[2];
 debug_err(("LIS331DL_Init ... \r\n"));
#if 1
    //buf[0] = CTRL_REG2;         buf[1] = 0x5e;  LIS331DL_Write2Byte(buf);
    buf[0] = CTRL_REG3;         buf[1] = I1CFG0;  LIS331DL_Write2Byte(buf);
    buf[0] = FF_WU_CFG_1;       buf[1] = (ZHIE | YHIE | XHIE);  LIS331DL_Write2Byte(buf);
    buf[0] = CTRL_REG1;         buf[1] = ( DR|PD | FS| ZEN | YEN | XEN);  LIS331DL_Write2Byte(buf);	
    buf[0] = FF_WU_THS_1;       buf[1] = 0x50;  LIS331DL_Write2Byte(buf);
#endif
...
void LIS331DL_SetSensitivity(UINT32 level)
{
 	UINT32 buf[2];

      debug_err(("LIS331DL_SetSensitivity---%d---\r\n",level));
	...
	{
		case 0: //off
			buf[0] = CTRL_REG1;       buf[1] = 0x00;  LIS331DL_Write2Byte(buf);			
		break;
		case 1://low	
			buf[0] = CTRL_REG3;         buf[1] = I1CFG0;  LIS331DL_Write2Byte(buf);
			buf[0] = FF_WU_CFG_1;       buf[1] = (ZHIE | YHIE | XHIE);  LIS331DL_Write2Byte(buf);
			buf[0] = CTRL_REG1;         buf[1] = ( DR|PD | FS| ZEN | YEN | XEN);  LIS331DL_Write2Byte(buf);	
			buf[0] = FF_WU_THS_1;       buf[1] = 0x37;  LIS331DL_Write2Byte(buf);			
			buf[0] = FF_WU_DURATION_1;       buf[1] = 0x2f;  LIS331DL_Write2Byte(buf);				
		break;
		case 2://med
			buf[0] = CTRL_REG3;         buf[1] = I1CFG0;  LIS331DL_Write2Byte(buf);
			buf[0] = FF_WU_CFG_1;       buf[1] = (ZHIE | YHIE | XHIE);  LIS331DL_Write2Byte(buf);
			buf[0] = CTRL_REG1;         buf[1] = ( DR|PD | FS| ZEN | YEN | XEN);  LIS331DL_Write2Byte(buf);	
			buf[0] = FF_WU_THS_1;       buf[1] = 0x20;  LIS331DL_Write2Byte(buf);			
			buf[0] = FF_WU_DURATION_1;       buf[1] = 0x2f;  LIS331DL_Write2Byte(buf);		
		break;
		case 3:
			buf[0] = CTRL_REG3;         buf[1] = I1CFG0;  LIS331DL_Write2Byte(buf);
			...
			buf[0] = FF_WU_DURATION_1;       buf[1] = 0x20;  LIS331DL_Write2Byte(buf);		
		break;
		default:
		break;		
	}
...
void LIS331DL_ReadInterupt()
{
	UINT32 buf[2];
	buf[0] = 0x31; LIS331DL_Write1BRead1B(buf);		
}
...
GSENSOR_FUNC LIS331DLFP = {
    ...
    LIS331DL_SetSensitivity, //set sensitivity
    LIS331DL_ReadInterupt, //read interupt when interupte happended
};
