...
extern UINT32 app_check_reckey_space(void);
void app_reset_reckey_count(void);
extern void app_set_boot_rec_start(void);
...
//extern UINT8 SP_1[12];
//static CHAR  SP_buf[20];
//static UINT32  SP_buf[20];
...
static void FlowMovie_StopRec(BOOL stopLib)
{
    ...
    if(gbKscanTVPlug == FALSE){		
	  //KeyScan_EnableLEDToggle(KEYSCAN_LED_RED, FALSE);
...
//----------------------jack lan begin-----------------------------------------
	...
	if(speed_hi <= 200)
	{
    	sprintf(Speed_buf,"%03d km/h",speed_hi);
...
INT32 UIFlowWndMovie_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
   static BOOL first = TRUE;
    ...
    KeyScanSetDetTVEn(TRUE);	 // jacky lan @20150914 for G513
...
INT32 UIFlowWndMovie_OnMovieOneSec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    if (gUIAviIsRecording == TRUE)
    {
	 ...
	 {
        	UxCtrl_SetShow(&UIFlowWndMovie_PanelCtrl,0);
		GPIOMap_TurnOffLED(GPIOMAP_LED_GREEN);
	 }
	 else
	 {
		UxCtrl_SetShow(&UIFlowWndMovie_PanelCtrl,1);
...
INT32 UIFlowWndMovie_OnTimer(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    NVT_USER_EVT key;
    UINT32 speed_hi,speed_lo,speed_total;
    UINT32 year,year_tmp,month,day;
 ...
 //   debug_err(("UIFlowWndMovie_OnTimer\n\r"));
    if(Get_DisplayIndex() != DISPLAY_OFF)
    {
		...
		Counter++;
		//app_set_tv_detect_space_count_add();
            if (Get_MovieGPSIndex()==MOVIE_GPS_ON)
	         ...
	         if(GPSSetTime == TRUE)
		  {
			   ...
			   if(GPSTime_H > 23)
			   {
				...
				switch(GPSTime_M)
				{
					...
					break;						
					case 2:
						if(rtc_isLeapYear(year_tmp)==TRUE)
						{
							if(GPSTime_D==29)
							{
								GPSTime_D=1;
