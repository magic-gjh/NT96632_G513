...
#if 1//(_NAVI_COMUNICATION_PRO_ == ENABLE)
    // Init UART
    uart2_open();
    // Set UART parameter
...
//#NT#2009/05/20#Corey Ke -end
    ...
    if (uiShowLogo == LOGO_STARTUP) {
        ...
        DecOneJPGInfo.JPGFileSize       = BGDB_GetPowerOnBGSize();
    } else if (uiShowLogo == LOGO_POWEROFF) {
        DecOneJPGInfo.pSrcAddr          = (UINT8 *)g_ucBGGoodbye;
        DecOneJPGInfo.JPGFileSize       = BGDB_GetPowerOffBGSize();
...
/**
  ...
  @return void
*/
void SysInit_DisplaySplashScreen2(UINT32 uiShowLogo,UINT8 *pBgBuf, UINT32 uiSize)
{
//#NT#2009/05/20#Corey Ke -begin
//#NT#Refine for 96630
    UINT32              uiPoolAddr;//, uiFlag;
//#NT#2009/05/20#Corey Ke -end
    ...
    DecOneJPGInfo.pJPGScalarHandler = NULL;
    if (uiShowLogo == LOGO_STARTUP) {
        DecOneJPGInfo.pSrcAddr          = (UINT8 *)g_ucBGOpening2;
        DecOneJPGInfo.JPGFileSize       = BGDB_GetPowerOnBGSize2();
    } else if (uiShowLogo == LOGO_POWEROFF) {
        DecOneJPGInfo.pSrcAddr          = (UINT8 *)g_ucBGGoodbye;
        DecOneJPGInfo.JPGFileSize       = BGDB_GetPowerOffBGSize();
    } else if (uiShowLogo == LOGO_GOD) {
    ...
    if (FALSE == UI_ShowLogoFile(PS_BG_LOGO))
     {
        ...
        //TimerDelayMs(800);		
        //SysInit_DisplaySplashScreen2(LOGO_STARTUP,0,0);	  
     }
