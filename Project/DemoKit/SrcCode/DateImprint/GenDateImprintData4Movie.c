...
void MovieDateImprint_Setup(UINT32 flag)
{
    ...
    {
        sprintf(pDateImprintData->pi8Str, "0000/00/00 00:00:00 000 km/h");
    }
    else
    {
        sprintf(pDateImprintData->pi8Str, "0000/00/00 000 km/h");
    }
...
void MovieDateImprint_CopyData(UINT32 yAddr, UINT32 cbAddr, UINT32 crAddr, UINT32 lineY, UINT32 imgWidth)
{
    ...
    if (gCurTime.s.second != CurTime.s.second)
    {
        ...
        if ((g_uiMovieDateStampSetup & STAMP_DATE_TIME_MASK) == STAMP_DATE_TIME)
        {
            switch (g_uiMovieDateStampSetup & STAMP_DATE_FORMAT_MASK)
            {
            	case STAMP_DD_MM_YY:
		   			...
		   			{
                		sprintf(g_cMovieDateStampStr, "%02d/%02d/%04d %02d:%02d:%02d %03d km/h", CurDate.s.day, CurDate.s.month, CurDate.s.year, 
							CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi);
		   			}
		   			else
		   			{
            	...
            	case STAMP_MM_DD_YY:
		   			if(GPSDataInputFlag == TRUE)
		   			{
               	 		sprintf(g_cMovieDateStampStr, "%02d/%02d/%04d %02d:%02d:%02d %03d km/h", CurDate.s.month, CurDate.s.day, CurDate.s.year, 
							CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi);
		  			}
		   			else
		   			{
            	...
            	default:
		  			if(GPSDataInputFlag == TRUE)
		  			{
                		sprintf(g_cMovieDateStampStr, "%04d/%02d/%02d %02d:%02d:%02d %03d km/h", CurDate.s.year, CurDate.s.month, CurDate.s.day, 
							CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi);
					}
		  			else
		 			{
        ...
        else
        {
	     	if(GPSDataInputFlag == TRUE)
	   		{
            	sprintf(g_cMovieDateStampStr, "%02d:%02d:%02d %03d km/h", CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi);
	   		}
	   		else
	   		{
...
void MovieDateImprint_CopyData(UINT32 yAddr, UINT32 cbAddr, UINT32 crAddr, UINT32 lineY, UINT32 imgWidth)
{
    ...
    if (gCurTime.s.second != CurTime.s.second)
    {
        ...
        if ((g_uiMovieDateStampSetup & STAMP_DATE_TIME_MASK) == STAMP_DATE_TIME)
        {
		  ...
		  if(GPSDataInputFlag == TRUE){
		  //if(0){
                sprintf(g_cMovieDateStampStr, "%02d/%02d %02d:%02d:%02d  Speed: %03d.%d km/h  ", CurDate.s.day, CurDate.s.month,  
					CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi,speed_lo);}
		   else{
                sprintf(g_cMovieDateStampStr, "%02d/%02d/%04d %02d:%02d:%02d", CurDate.s.day, CurDate.s.month, CurDate.s.year, CurTime.s.hour, CurTime.s.minute, CurTime.s.second);
			}		   	
                break;
		   ...
		   if(GPSDataInputFlag == TRUE){	
		   //if(0){
                sprintf(g_cMovieDateStampStr, "%02d/%02d %02d:%02d:%02d  Speed: %03d.%d km/h  ", CurDate.s.month, CurDate.s.day, 
					CurTime.s.hour, CurTime.s.minute, CurTime.s.second,speed_hi,speed_lo);}
		   else{
                sprintf(g_cMovieDateStampStr, "%02d/%02d %02d:%02d:%02d", CurDate.s.month, CurDate.s.day, CurTime.s.hour, CurTime.s.minute, CurTime.s.second);
		   	}
                break;
            default:
		   if(GPSDataInputFlag == TRUE){
        ...
        {
	     if(GPSDataInputFlag == TRUE){			        
            sprintf(g_cMovieDateStampStr, "%02d:%02d:%02d  Speed: %03d.%d km/h  ", CurTime.s.hour, CurTime.s.minute, CurTime.s.second
				,speed_hi,speed_lo);}
		else
		{
            sprintf(g_cMovieDateStampStr, "%02d:%02d:%02d", CurTime.s.hour, CurTime.s.minute, CurTime.s.second);
