...
UINT8 app_check_boot_rec_start(void);
void app_set_boot_rec_start(void);
...
static UINT8 BootRecStart = 0;
#define GPS_TIME_SECOND_BIT   24
...
void app_Reset_boot_rec_start(void)
{
	BootRecStart = 0;;
}
...
void CommandTsk(void)
{
    ...
    static UINT32 GpsTime_Second_Last;
    ...
    while(1)
    {
	 ...
	 CheckSum=0;
	//if((app_check_boot_rec_start() == 0)||(ACC5MinMode == TRUE) || (ACCHalfHourMode == TRUE))
	if((app_check_boot_rec_start() == 0) || (GPIOMap_GetAcc12V() == FALSE))
	{
	...
	if(Step == 7)
	{
		...
		{
		      //debug_msg("string_buf[%d] : 0x%02x \r\n",i,string_buf[i]);					
			CheckSum+= string_buf[i];
		}
		//debug_msg("string_buf[21] : 0x%02x \r\n",string_buf[21]);
		CheckSum = (CheckSum&0xff);
		debug_msg("checksum_cal : 0x%02x , CheckSum_Cur:%x \r\n",CheckSum,CheckSum_Cur);			
		...
		if (CheckSum == CheckSum_Cur)
		{
			switch(CommandIndex)
			{
				case COMMAND_GPS_INFO:
					if((sp_buf[0] == 1)&&(GpsTime_Second_Last!=string_buf[GPS_TIME_SECOND_BIT])) // gps 
					{
					      GPSDataCounter++;
	...
	GpsTime_Second_Last=string_buf[GPS_TIME_SECOND_BIT];
